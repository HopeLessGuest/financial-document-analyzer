...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({
  ...
  return (
    <div className="bg-slate-50 border border-slate-200 shadow-sm rounded-lg p-4 sm:p-6 flex flex-col h-[500px] max-h-[70vh]">
      ...
      <form onSubmit={handleSubmit} className="flex items-center space-x-2 pt-3 border-t border-slate-200">
        <input
          type="text"
          value={inputText}
          onChange={(e) => setInputText(e.target.value)}
          placeholder={isLoading ? "AI is thinking..." : "Ask a question..."}
          className="flex-grow bg-white border border-slate-300 text-slate-800 placeholder-slate-400 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 p-2.5 transition-colors"
          disabled={isLoading}
        ...
        <button
          ...
          className="bg-blue-600 hover:bg-blue-700 text-white font-medium p-2.5 rounded-lg shadow-md transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
